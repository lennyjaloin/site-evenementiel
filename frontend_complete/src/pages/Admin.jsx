    ...
    <section className="container-app py-6 sm:py-8">
      <motion.div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-6"
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={()=>setTab("events")} className={`text-xs sm:text-sm ${tab==="events"?"btn-primary":"btn-secondary"}`}>Événements</button>
          <button onClick={()=>setTab("reservations")} className={`text-xs sm:text-sm ${tab==="reservations"?"btn-primary":"btn-secondary"}`}>Réservations</button>
          <button onClick={load} className="btn-ghost text-xs sm:text-sm">Rafraîchir</button>
        </div>
      </motion.div>
